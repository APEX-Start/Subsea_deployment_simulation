...
本文件作为当前工作区的主仿真脚本，目标不是保留某个单一历史版本的
"原样实现"，而是把已经验证过且彼此互补的能力收敛到一个可配置的工程基线。

版本定位:
1. `high_fidelity_sim.py`
   工程主版本。默认启用更稳健的离散阻力、事件停止、稀疏雅可比和平均指标。
2. `high_fidelity_1_opt.py`
   对照/校核版本。保留较直接的实现和更严格的求解参数风格，便于对比。
3. `high_fidelity_novibrant.py`
   展示/平滑版本。保留较强的去振与汇报导向后处理，适合做图，不宜直接用于设计峰值判据。

本文件额外提供若干"合并开关"，把另外两个版本中确实有用的行为收敛进来:
- `solver_profile`
  选择工程默认求解参数或更严格的 `1_opt` 风格求解参数。
- `initialization_mode`
  选择工程默认的海流偏置初始构型，或传统竖直初始构型。
